...
class HomeScreenTest {
  ...
  fun allDisplayedHomeScreenEntriesMustHaveAClickAction() {
    composeAndroidTestRule.apply {
      onNodeWithTag(TestTags.HOME_SCREEN_LIST).onChildren().assertAll(hasClickAction())
    }
  }
  ...
  fun allHomeScreenEntriesMustHaveTextFromTheDataProvider() {
    composeAndroidTestRule.apply {
      onNodeWithTag(TestTags.HOME_SCREEN_LIST)
        .onChildren()
        .assertAll(hasTextInProvidedDemoData(DemoDataProvider.homeScreenListItems))
    }
  }
  ...
  private fun hasTextInProvidedDemoData(
    ...
    SemanticsMatcher(
      description =
        "${SemanticsProperties.Text.name} is in '$homeScreenListItems' items' names (ignoreCase: $ignoreCase)"
    ) { semanticNode ->
