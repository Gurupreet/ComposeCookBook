...
class SystemUiController(private val window: Window) {
   ...
   * @param darkIcons Whether dark status bar icons would be preferable. Only available on API 23+.
   ...
   * @param darkIcons Whether dark navigation bar icons would be preferable. Only available on API
   *   26+.
   * @param transformColorForLightContent A lambda which will be invoked to transform [color] if
   *   dark icons were requested but are not available. Defaults to applying a black scrim.
   */
...
val LocalSystemUiController =
  staticCompositionLocalOf<SystemUiController> { error("No SystemUiController provided") }
