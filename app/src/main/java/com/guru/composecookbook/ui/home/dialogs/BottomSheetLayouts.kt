...
fun BottomSheetDrawer() {
  ...
  DismissibleNavigationDrawer(
    drawerContent = {
      Surface(modifier = Modifier.fillMaxSize().padding(end = 16.dp)) { DrawerContent() }
    },
    content = {
...
@Composable
private fun ScafoldContent(coroutineScope: CoroutineScope, showBottomSheet: (Boolean) -> Unit) {
...
fun FullBottomSheetContent(onClose: () -> Unit) {
  Surface(modifier = Modifier.fillMaxSize().background(Color.Transparent).padding(top = 8.dp)) {
    Column(
