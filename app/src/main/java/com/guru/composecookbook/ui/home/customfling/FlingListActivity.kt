...
class FlingListActivity : ComponentActivity() {
  ...
  companion object {
    ...
    fun newIntent(context: Context, isDarkTheme: Boolean) =
      Intent(context, FlingListActivity::class.java).apply { putExtra(DARK_THEME, isDarkTheme) }
  }
      ...
      SmallTopAppBar(
        title = {
          Column(modifier = Modifier.padding(4.dp)) {
             ...
             */
            Text(text = "Custom Fling", modifier = Modifier.testTag(TestTags.HOME_FLING_HEADER))
          }
        },
        navigationIcon = {
...
fun DefaultPreview2() {
  ComposeCookBookTheme {
    val state =
      MutableLiveData(FlingStateStore.INSTANCE.copy())
        .observeAsState(FlingStateStore.INSTANCE.copy())

    ListViewContent(DemoDataProvider.itemList, state, onBack = {}) {}
  }
}
