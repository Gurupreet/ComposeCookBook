...
fun SimpleColorStateAnimation() {
  ...
  val animatedColor =
    animateColorAsState(
      targetValue =
        if (enabled.value) MaterialTheme.colorScheme.primary
...
fun SimpleDpStateAnimations() {
  ...
  Button(
    ...
    colors = buttonColors,
    modifier =
      Modifier.padding(16.dp).height(animatedHeightState.value).width(animatedWidthState.value),
  ) {
    Text("Scale & Color")
  }
}
...
fun SimpleOffsetStateAnimation() {
  ...
  val animatedOffset by
    animateOffsetAsState(targetValue = if (enabled) Offset(0f, 0f) else Offset(50f, 40f))
