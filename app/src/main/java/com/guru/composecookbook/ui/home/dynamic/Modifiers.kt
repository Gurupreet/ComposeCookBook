  ...
  Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
    ...
    DemoElementText(
      modifier =
        Modifier.padding(8.dp)
          .background(
            brush =
              Brush.horizontalGradient(colors = listOf(teal200, green500), startX = 0f, endX = 100f)
          )
    )
    ...
    DemoElementText(
      modifier =
        Modifier.background(
            brush =
              Brush.horizontalGradient(colors = listOf(teal200, green500), startX = 0f, endX = 200f)
          )
          .padding(8.dp)
    )
    ...
    DemoElementText(
      modifier =
        Modifier.background(
            brush =
              Brush.verticalGradient(colors = listOf(teal200, green500), startY = 0f, endY = 500f)
          )
          .padding(8.dp)
    )

    DemoText("Modifier.background(teal200).padding(8.dp).clickable(onClick = {})")
    DemoElementText(modifier = Modifier.background(teal200).padding(8.dp).clickable(onClick = {}))
    DemoText("Modifier.clickable(onClick = {}).background(teal200).padding(8.dp)")
    DemoElementText(modifier = Modifier.clickable(onClick = {}).background(teal200).padding(8.dp))
    DemoText("Add double tap: Modifier.pointerInteropFilter for tap and motion events ")
    DemoElementText(
      modifier = Modifier.background(teal200).pointerInteropFilter { true }.padding(8.dp)
    )
    DemoText("Use Modifier.pointerInput() for all drag, tag, long press gestures")
    DemoElementText(
      modifier =
        Modifier.background(teal200).pointerInput("key") { this.detectTapGestures {} }.padding(8.dp)
    )
  }
}
...
fun DemoElementButton(modifier: Modifier) {
  Button(onClick = {}, modifier = modifier) { Text(text = "Basic Button") }
}
...
fun DemoText(text: String) {
  Text(text = text, style = typography.body2.copy(Color.Gray), modifier = Modifier.padding(4.dp))
}
