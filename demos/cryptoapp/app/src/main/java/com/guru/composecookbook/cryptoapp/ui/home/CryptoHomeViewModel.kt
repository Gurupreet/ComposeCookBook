...
class CryptoHomeViewModel(context: Context) : ViewModel() {
  ...
  fun getAllCryptos(pageSize: Int = 20) =
    Pager(config = PagingConfig(pageSize = pageSize, initialLoadSize = pageSize)) {
        PageNumSource { pageNo, pageSize -> cryptoRepository.getPageCryptos(pageNo, pageSize) }
      }
      .flow
      .cachedIn(viewModelScope)

  fun addToFav(crypto: Crypto) {
    viewModelScope.launch(Dispatchers.IO) { cryptoRepository.addFavorite(crypto = crypto) }
  }

  fun removeFav(crypto: Crypto) {
    viewModelScope.launch(Dispatchers.IO) { cryptoRepository.removeFavorite(crypto = crypto) }
  }
