...
class MovieRepositoryImpl(
  ...
  override suspend fun getNowShowing(): Flow<List<Movie>> =
    flow {
      ...
      .catch { e -> emit(emptyList<Movie>()) }
      .flowOn(Dispatchers.IO)

  override suspend fun getSimilarMovies(movieId: String): Flow<List<Movie>> =
    flow {
  ...
  override suspend fun fetchAndSaveGenresToDatabase(): Flow<List<Genre>> =
    flow {
