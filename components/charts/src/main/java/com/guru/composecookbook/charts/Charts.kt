...
fun createRandomFloatList(): List<Float> {
  ...
  (0..20).forEach { _ -> list.add(Random.nextFloat() * 10) }
  return list
}
...
fun LineChart(
  ...
  LaunchedEffect(animationKey) {
    x.animateTo(
      targetValue = xTarget,
      animationSpec = tween(durationMillis = if (shouldAnimate) 500 else 0, easing = LinearEasing),
    )
  }
...
fun PieCharts(pieValues: List<Float>, shouldAnimate: Boolean = true) {
  ...
  LaunchedEffect(Unit) {
    index.animateTo(
      ...
      animationSpec =
        tween(durationMillis = if (shouldAnimate) 300 else 0, easing = FastOutSlowInEasing),
    )
  }
...
fun DrawScope.drawBar(topLeft: Offset, width: Float, height: Float, colors: List<Color>) {
  drawRect(topLeft = topLeft, brush = Brush.linearGradient(colors), size = Size(width, height))
}
