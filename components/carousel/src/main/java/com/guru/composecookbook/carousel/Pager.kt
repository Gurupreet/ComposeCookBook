...
fun Pager(
  ...
  Layout(
    content = { state.PrepareContent(offscreenLimit = offscreenLimit) { content.invoke(this) } },
    modifier =
      modifier.draggable(
        orientation = orientation,
        onDragStarted = { state.selectionState = SelectionState.Undecided },
        onDragStopped = { velocity ->
          coroutineScope.launch {
            // Velocity is in pixels per second, but we deal in percentage offsets, so
            // we
...
private fun PagerState.PrepareContent(
  ...
  for (page in minPage..maxPage) {
    ...
    key(pageData) { Column(modifier = pageData) { scope.content() } }
  }
}
